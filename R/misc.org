...
#+BEGIN_SRC R
  ...
  pairRows <- function(dat1, ID) {
    ...
    combos<-combn(dat1[[ID]], 2)
    dat2 <- data.frame(ID.1=combos[1,], ID.2=combos[2,])
    for (i in 2:ncol(dat)) {
      dat2[paste0(colnames(dat1)[i], ".1")] <- dat1[combos[1,], i]
      dat2[paste0(colnames(dat1)[i], ".2")] <- dat1[combos[2,], i]
    }
    dat2
  }
#+END_SRC
